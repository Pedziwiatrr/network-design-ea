    ...
    try:
        ...
        if args.test_loading or args.all:
            ...
            for demand in network.demands.values():
                print(f"Demand: {demand.id} | Route: {demand.source} -> {demand.target} | Value: {demand.value}")
        ...
        if args.test_solver or args.all:
            ...
            print(f"Result: Cost {best_cost}, Last improvement in gen: {last_improvement_gen}")
            print("="*100)
